...
MComPtr<ID3DBlob> myFirstMeshShader;
...
MComPtr<ID3D12PipelineState> MSPipelineState; // VkPipeline -> ID3D12PipelineState
...
MComPtr<ID3D12Resource> primitiveIndiceBuffer;
	...
	if (true)
	{
		...
		// Renderer
		{
			...
			if (true)
			{
				...
				{
					std::cout << "Shader models not supported !" << std::endl;
				}
				...
				// Lit
				{
						...
						const D3D12_ROOT_PARAMETER1 params[]{
							...
							// Point Lights Structured buffer
							{
								...
								*/
								...
								{
								 .ParameterType = D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE,
								 .DescriptorTable{.NumDescriptorRanges = _countof(pbrTextureRange),
...
#else
					{
						// Mesh Shader
						{
							...
							const HRESULT hrCompileShader = D3DReadFileToBlob(L"Resources/Shaders/HLSL/MeshShader.cso", &myFirstMeshShader);

							if (FAILED(hrCompileShader))
							{
								std::string errorStr(static_cast<const char*>(errors->GetBufferPointer()), errors->GetBufferSize());
								SA_LOG(L"Shader {MeshShader.hlsl, main} compilation failed!", Error, DX12, errorStr);
								return EXIT_FAILURE;
							}
							else
								SA_LOG(L"Shader {MeshShader.hlsl, main} compilation success.", Info, DX12, myFirstMeshShader.Get());
						}
					}
					// Fragment Shader
					{
						const HRESULT hrCompileShader = D3DReadFileToBlob(L"Resources/Shaders/HLSL/LitShader.cso", &litPixelShader);
						//const HRESULT hrCompileShader = D3DCompileFromFile(L"Resources/Shaders/HLSL/LitShader.hlsl", nullptr, nullptr, "mainPS", "ps_6_0", shaderCompileFlags, 0, &litPixelShader, &errors);
						if (FAILED(hrCompileShader))
						{
							std::string errorStr(static_cast<const char*>(errors->GetBufferPointer()), errors->GetBufferSize());
							SA_LOG(L"Shader {LitShader.hlsl, mainPS} compilation failed.", Error, DX12, errorStr);
...
#else
						const D3DX12_MESH_SHADER_PIPELINE_STATE_DESC MSdesc{
							.pRootSignature = litRootSign.Get(),

							.MS{
								.pShaderBytecode = myFirstMeshShader->GetBufferPointer(),
								.BytecodeLength = myFirstMeshShader->GetBufferSize()
							},
							.PS{
								.pShaderBytecode = litPixelShader->GetBufferPointer(),
								.BytecodeLength = litPixelShader->GetBufferSize()
...
#endif
						CD3DX12_PIPELINE_MESH_STATE_STREAM castedMSdesc = CD3DX12_PIPELINE_MESH_STATE_STREAM(MSdesc);
						D3D12_PIPELINE_STATE_STREAM_DESC strDesc = {};
						strDesc.pPipelineStateSubobjectStream = &castedMSdesc;
						strDesc.SizeInBytes = sizeof(castedMSdesc);
#ifndef D3DX_MESH_SHADER
...
#else
						const HRESULT hrCreateMSPipeline = device->CreatePipelineState(&strDesc, IID_PPV_ARGS(&MSPipelineState));
						if (FAILED(hrCreateMSPipeline))
						{
							SA_LOG(L"Create MeshShader PipelineState failed!", Error, DX12, (L"Error Code: %1", hrCreateMSPipeline));
							return EXIT_FAILURE;
						}
						else
						{
							SA_LOG(L"Create MeshShader PipelineState success.", Info, DX12, litPipelineState.Get());
						}
#endif
					}
				}
				...
				// Meshes
				{
					// Sphere
					{
						...
						std::vector<uint32_t> indices;
						{
							...
							const UINT indexCount = inMesh->mNumFaces * 3;
							...
							HRESULT hrBufferCreated = device->CreateCommittedResource(
								&heap, D3D12_HEAP_FLAG_NONE, &desc,
								D3D12_RESOURCE_STATE_COMMON,
								nullptr, IID_PPV_ARGS(&sphereIndexBuffer));
							...
							{
								indices[i * 3 + 0] = inMesh->mFaces[i].mIndices[0];
							...
							const bool bSubmitSuccess = SubmitBufferToGPU(
								sphereIndexBuffer, bufferSize,
								indices.data(), D3D12_RESOURCE_STATE_INDEX_BUFFER);
						...
						size_t nFaces = inMesh->mNumFaces;
						size_t nVerts = inMesh->mNumVertices;
						...
						std::vector<DirectX::MeshletTriangle> primitiveIndices;
						{
							...
							if (FAILED(ComputeMeshlets(indices.data(), nFaces, pos.get(), nVerts,
								...
								primitiveIndices)))
							{
								SA_LOG(L"ComputeMeshlets failed", Error, DX12);
								return EXIT_FAILURE;
							}
							...
							const HRESULT hrBufferCreated = device->CreateCommittedResource(
								...
								nullptr, IID_PPV_ARGS(&meshletBuffer));
							if (FAILED(hrBufferCreated))
							{
							...
							else
							{
								...
								SA_LOG(L"Create Meshlets Buffer success", Info, DX12,
									(L"\"%1\" [%2]", name, meshletBuffer.Get()));
							}

							const bool bSubmitSuccess =
								SubmitBufferToGPU(meshletBuffer, desc.Width, meshlets.data(),
						...
						}
						{
							...
							const D3D12_RESOURCE_DESC desc{
								.Dimension = D3D12_RESOURCE_DIMENSION_BUFFER,
								.Alignment = 0,
								.Width = nFaces * sizeof(unsigned int),
								.Height = 1,
								.DepthOrArraySize = 1,
								.MipLevels = 1,
								.Format = DXGI_FORMAT_UNKNOWN,
							...
							const HRESULT hrBufferCreated = device->CreateCommittedResource(
								&heap, D3D12_HEAP_FLAG_NONE, &desc, D3D12_RESOURCE_STATE_COMMON,
								nullptr, IID_PPV_ARGS(&primitiveIndiceBuffer));
							if (FAILED(hrBufferCreated))
							{
							...
							else
							{
								...
								primitiveIndiceBuffer->SetName(name);

								SA_LOG(L"Create Meshlets Primitive Indices Buffer success", Info,
									DX12, (L"\"%1\" [%2]", name, primitiveIndiceBuffer.Get()));
							}

							const bool bSubmitSuccess = SubmitBufferToGPU(
								primitiveIndiceBuffer, desc.Width, primitiveIndices.data(),
...
#else
						cmd->SetGraphicsRootDescriptorTable(6, gpuHandle);
						cmd->SetGraphicsRootShaderResourceView(
							2, meshletBuffer->GetGPUVirtualAddress()); // Meshlets
						cmd->SetGraphicsRootShaderResourceView(
							3, sphereVertexBuffers[0]->GetGPUVirtualAddress()); // Vertex Buffer
						cmd->SetGraphicsRootShaderResourceView(
							4, sphereIndexBuffer->GetGPUVirtualAddress()); // Index Buffer
						cmd->SetGraphicsRootShaderResourceView(
							5, primitiveIndiceBuffer->GetGPUVirtualAddress()); // Primitive Indices

						/* 0008-U */
						cmd->SetPipelineState(MSPipelineState.Get());
	...
	if(true)
	{
		// Renderer
		{
			...
			// Scene Objects /* 0009-D */
			{
				...
				//  Meshlet Buffer
				{
					SA_LOG(L"Destroying Meshlet Buffer...", Info, DX12, meshletBuffer.Get());
					meshletBuffer = nullptr;
				}

				// Primitive Index Buffer
				{
					SA_LOG(L"Destroying Primitive Index Buffer...", Info, DX12, primitiveIndiceBuffer.Get());
					primitiveIndiceBuffer = nullptr;
				}
			...
			// Pipeline /* 0008-D */
			{
				// Lit
				{
					...
					// MS PipelineState
					{
						SA_LOG(L"Destroying MS PipelineState...", Info, DX12, MSPipelineState.Get());
						MSPipelineState = nullptr;
					}

					// Pixel Shader
					{
						SA_LOG(L"Destroying Lit Pixel Shader...", Info, DX12, litPixelShader.Get());
						litPixelShader = nullptr;
					}

					// Mesh Shader
					{
						SA_LOG(L"Destroying Mesh Shader...", Info, DX12, myFirstMeshShader.Get());
						myFirstMeshShader = nullptr;
					}
